...
<div class="bg-gsb-blue text-white py-1 px-8 flex justify-between items-center shadow-md font-ui">
    <!-- Logo -->
    <div>
        <a href="<?= $basePath ?>templates/comptable.php">
    ...
    <nav class="flex-grow flex justify-center space-x-10 text-base">
        <?php if ($currentPage !== 'comptable.php'): ?>
            <a href="<?= $basePath ?>templates/comptable.php"
               class="hover:text-gsb-light transition <?= $currentPage === 'comptable.php' ? 'font-bold underline' : '' ?>">
                Accueil
            </a>
        <?php endif; ?>

        <?php if ($currentPage !== 'gestion_remboursement.php'): ?>
            <a href="<?= $basePath ?>views/fiches/gestion_remboursement.php"
